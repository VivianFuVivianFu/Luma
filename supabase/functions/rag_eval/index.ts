...
serve(async (req) => {
  try {
    ...
    console.log(`🔍 RAG Eval request: ${req.method} from ${req.headers.get('user-agent')}`)
    console.log(`Forwarding to: ${API_BASE}/api/rag/eval`)
    ...
    const response = await fetch(`${API_BASE}/api/rag/eval`, {
      method: req.method,
      headers: {
    ...
    console.error('❌ RAG Eval function error:', error)
    return new Response(
      JSON.stringify({ 
